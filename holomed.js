/* =====================================================================
HOLOMED — INTEGRAÇÃO CLOUDFLARE WORKER (projetoinfosaudejovemccb)
===================================================================== */
(function () {
  "use strict";

  // URL do Worker Cloudflare (sem barra final)
  ...
  var variacaoPose = Math.random() * 100;
  ...
  /* --- INTEGRAÇÃO WORKER CLOUDFLARE --- */
  async function perguntarWorker(pergunta) {
    var ctrl = new AbortController();
    var t = setTimeout(function () { ctrl.abort(); }, 30000); // 30s timeout igual worker
    ...
    try {
      ...
      if (!r.ok) {
        ...
        console.error("Worker error:", r.status, errData);
        return null;
      }

      var d = await r.json();

      // Resposta unificada do Worker: { success, provider, model, response, usage }
      if (d.success && typeof d.response === "string" && d.response.trim()) {
        return d.response.trim();
      }

      console.warn("Worker response invalid:", d);
      return null;
  ...
  form.addEventListener("submit", async function (ev) {
    ...
    var ia = await perguntarWorker(texto);
    if (ia) {
  ...
  function iniciar3D() {
    ...
    function braco(x) {
      ...
      antebraco.position.y = -0.52; g.add(Código frontend atualizado. Integra Worker via endpoint `/v1/chat` e resposta unificada.

Substitua função `perguntarVireonix` e ajuste URL:
